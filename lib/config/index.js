...
const assertMetaInfoBaseUrls = (metaInfoBaseUrls) => {
    ...
    if (!metaInfoBaseUrls.file) {
        return;
    }

    if (typeof metaInfoBaseUrls.file === 'string') {
    ...
    if (!_.isObject(metaInfoBaseUrls.file) && !_.isString(metaInfoBaseUrls.file)) {
        throw new Error(`"metaInfoBaseUrls.file" option must be string or object, but got ${typeof metaInfoBaseUrls.file}`);
    }

    for (const key in metaInfoBaseUrls.file) {
        if (key !== 'url' && key !== 'query') {
            throw new Error(`"metaInfoBaseUrls.file" option must contain fields "url" and "query", but got ${key}`);
        }

        if (key === 'url' && !_.isString(metaInfoBaseUrls.file[key])) {
            throw new Error(`Value of "${key}" in "metaInfoBaseUrls" must be string, but got ${typeof metaInfoBaseUrls.file[key]}`);
        }

        if (key === 'query' && !_.isPlainObject(metaInfoBaseUrls.file[key])) {
            throw new Error(`Value of "${key}" in "metaInfoBaseUrls" must be object, but got ${typeof metaInfoBaseUrls.file[key]}`);
        }
    }

    for (const key in metaInfoBaseUrls.file.query) {
        if (!_.isString(metaInfoBaseUrls.file.query[key])) {
            throw new Error(`Value of "${key} in "metaInfoBaseUrls.file.query" must be string, but got ${typeof metaInfoBaseUrls.file.query[key]}`);
        }
    }
};
