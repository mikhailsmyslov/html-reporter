...
const metaToElements = (metaInfo, metaInfoBaseUrls) => {
    return map(metaInfo, (value, key) => {
        if (isUrl(value)) {
            ...
            if (typeof metaInfoBaseUrls[key] === 'string') {
            ...
            } else if (typeof metaInfoBaseUrls[key] === 'object') {
                let urlObject = metaInfoBaseUrls[key];
                const baseUrl = urlObject.url;
                let queries = '';
                if (urlObject.query) {
                    queries = '?' + Object.keys(urlObject.query)
                        .map((queryKey) => [queryKey, urlObject.query[queryKey]].join('='))
                        .join('&');
                }
                const link = isUrl(baseUrl) ? url.resolve(baseUrl, value + queries) : path.join(baseUrl, value + queries);
                value = mkLinkToUrl(link, value);
            }
